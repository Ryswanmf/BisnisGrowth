                    ...
                    <div class="sticky top-24 space-y-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <nav class="flex flex-col gap-0.5">
                                <a href="#" class="flex items-center justify-between px-3 py-2 rounded-xl bg-burgundy-600 text-white shadow-md shadow-burgundy-600/10 transition-all">
                                    <span class="text-[9px] font-bold uppercase tracking-widest">Semua</span>
                                    <span class="text-[9px] font-black opacity-60">{{ $articles->total() }}</span>
                                </a>
                                @foreach(['Marketing', 'Keuangan', 'Operasional', 'Teknologi', 'Legalitas'] as $cat)
                                <a href="#" class="flex items-center justify-between px-3 py-2 rounded-xl hover:bg-white text-gray-500 hover:text-burgundy-600 group transition-all border border-transparent hover:border-gray-100">
                                    <span class="text-[9px] font-bold uppercase tracking-widest">{{ $cat }}</span>
                                    <span class="text-[9px] font-black text-gray-300 group-hover:text-gold-500">12</span>
                                </a>
                                @endforeach
                            </nav>
                        </div>
                ...
                <div class="flex-grow">
                    <div class="flex items-center justify-between mb-8 px-2">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <select class="bg-transparent border-none text-[10px] font-black text-burgundy-900 uppercase tracking-widest focus:ring-0 cursor-pointer">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
                        ...
                        <article class="group flex flex-col bg-white rounded-3xl overflow-hidden border border-gray-100 transition-all duration-500 hover:shadow-2xl hover:shadow-burgundy-900/5 hover:-translate-y-1 h-full">
                            <a href="/artikel/{{ $article->slug }}" class="relative aspect-[16/10] overflow-hidden block shrink-0">
                                <img src="{{ $article->image }}" alt="{{ $article->title }}" 
                                     class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                                <div class="absolute top-4 left-4">
                                    <span class="px-3 py-1 bg-white/95 backdrop-blur-sm text-[9px] font-black uppercase tracking-widest text-burgundy-800 rounded-lg shadow-sm border border-gray-100">
                            ...
                            <div class="p-7 flex-grow flex flex-col">
                                <div class="flex items-center text-gray-400 text-[9px] font-bold uppercase tracking-[0.15em] mb-4">
                                    <span>{{ $article->published_at->format('d M Y') }}</span>
                                    <span class="mx-2 text-gold-400">•</span>
                                    <span>{{ ceil(str_word_count(strip_tags($article->content)) / 200) }} Menit</span>
                                </div>
                                
                                <h3 class="text-lg font-black text-burgundy-900 mb-3 leading-snug group-hover:text-burgundy-600 transition-colors line-clamp-2">
                                    <a href="/artikel/{{ $article->slug }}">{{ $article->title }}</a>
                                </h3>
                                
                                <p class="text-gray-500 text-xs leading-relaxed line-clamp-3 mb-6 opacity-80">
                                    {{ $article->excerpt }}
                                </p>

                                <div class="mt-auto pt-6 border-t border-gray-50">
                                    <a href="/artikel/{{ $article->slug }}" class="inline-flex items-center text-[10px] font-black text-burgundy-600 uppercase tracking-[0.2em] hover:text-burgundy-800 group/link transition-colors">
                                        Baca Artikel
                                        <svg class="w-3.5 h-3.5 ml-2 transition-transform group-hover/link:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                                    </a>
                                </div>
                            </div>
                        </article>
