...
@section('header', 'Otomatisasi Tautan Internal')
...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <a href="{{ route('admin.internal-links.create') }}" class="bg-amber-500 text-slate-900 px-6 py-3 rounded-lg text-sm font-black flex items-center gap-2 hover:bg-amber-400 transition-all shadow-lg shadow-amber-500/20">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors group">
                    <td class="px-6 py-4">
                        <span class="text-sm font-bold text-slate-900">{{ $link->keyword }}</span>
                    </td>
                    <td class="px-6 py-4">
                        <a href="{{ $link->url }}" target="_blank" class="text-xs text-amber-600 font-bold hover:underline">{{ $link->url }}</a>
                    </td>
                    <td class="px-6 py-4">
                        <span class="text-xs font-black text-slate-400">{{ $link->limit }}x</span>
                    </td>
                    <td class="px-6 py-4">
                        @if($link->is_active)
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('admin.internal-links.edit', $link) }}" class="p-2 bg-white border border-gray-100 rounded-md text-gray-400 hover:text-amber-500 transition-all">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            <form id="delete-form-{{ $link->id }}" action="{{ route('admin.internal-links.destroy', $link) }}" method="POST">
                                @csrf @method('DELETE')
                                <button type="button" onclick="confirmDelete('delete-form-{{ $link->id }}')" class="p-2 bg-white border border-gray-100 rounded-md text-gray-400 hover:text-red-500 transition-all">
