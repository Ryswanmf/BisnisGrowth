...
@section('title', 'Edit Pool Keyword')
@section('header', 'Edit Keyword Pool')
...
<div class="max-w-2xl mx-auto">
    ...
    <form action="{{ route('admin.short-keywords.update', $shortKeyword->id) }}" method="POST" class="bg-white p-8 rounded-xl shadow-sm border border-gray-100 space-y-6">
        @csrf
        @method('PUT')
        <div>
            <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Judul (Title)</label>
            <input type="text" name="title" value="{{ old('title', $shortKeyword->title) }}" class="w-full bg-gray-50 border-gray-100 rounded-lg px-4 py-3 focus:ring-amber-500 focus:border-amber-500 @error('title') border-red-500 @enderror" required>
        ...
        <div>
            ...
            <textarea name="description" rows="10" class="w-full bg-gray-50 border-gray-100 rounded-lg px-4 py-3 focus:ring-amber-500 focus:border-amber-500 @error('description') border-red-500 @enderror" required>{{ old('description', $shortKeyword->description) }}</textarea>
        ...
        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
            ...
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $shortKeyword->is_active ? 'checked' : '' }}>
