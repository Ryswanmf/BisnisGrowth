    ...
    <form action="{{ isset($page) ? route('admin.pages.update', $page) : route('admin.pages.store') }}" method="POST" class="space-y-8">
        ...
        <div class="bg-white p-8 rounded-lg shadow-sm border border-gray-100 space-y-6">
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $page->title ?? '') }}" placeholder="Contoh: Kebijakan Privasi" class="w-full bg-gray-50 border-none rounded-lg px-5 py-3 text-sm focus:ring-2 focus:ring-amber-500" required>
            </div>

            <div>
                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Konten Halaman</label>
                <textarea name="content" rows="15" class="w-full bg-gray-50 border-none rounded-lg px-5 py-3 text-sm focus:ring-2 focus:ring-amber-500" required>{{ old('content', $page->content ?? '') }}</textarea>
        ...
        <div class="bg-white p-8 rounded-lg shadow-sm border border-gray-100 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="meta_title" value="{{ old('meta_title', $page->meta_title ?? '') }}" class="w-full bg-gray-50 border-none rounded-lg px-5 py-3 text-sm focus:ring-2 focus:ring-amber-500">
            </div>

            <div>
                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Meta Description</label>
                <textarea name="meta_description" rows="3" class="w-full bg-gray-50 border-none rounded-lg px-5 py-3 text-sm focus:ring-2 focus:ring-amber-500">{{ old('meta_description', $page->meta_description ?? '') }}</textarea>
            </div>
        </div>
