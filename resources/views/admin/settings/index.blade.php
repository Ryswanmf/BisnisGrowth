...
<div class="max-w-5xl" x-data="{ tab: 'identity' }">
    @if(session('success'))
        <div class="mb-8 bg-emerald-50 border border-emerald-100 text-emerald-600 px-6 py-4 rounded-xl text-sm font-bold animate-pulse">
            {{ session('success') }}
        </div>
    @endif

    <!-- Tab Navigation -->
    <div class="flex gap-2 mb-8 bg-gray-100 p-1.5 rounded-xl w-fit">
        ...
        <button @click="tab = 'maintenance'" :class="tab === 'maintenance' ? 'bg-white text-slate-900 shadow-sm' : 'text-gray-400 hover:text-gray-600'" class="px-6 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all">Pemeliharaan</button>
    </div>

    <form action="{{ route('admin.settings.update') }}" method="POST" class="space-y-8">
...
        <!-- Tab: Maintenance -->
        <div x-show="tab === 'maintenance'" x-transition class="bg-white p-8 md:p-12 rounded-xl border border-gray-100 shadow-sm space-y-10">
            <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="p-8 bg-slate-50 rounded-[2rem] border border-gray-100 space-y-4">
                    <div class="h-12 w-12 bg-blue-500 rounded-xl flex items-center justify-center text-white shadow-lg shadow-blue-500/20">
                    ...
                    <a href="/sitemap.xml" target="_blank" class="inline-block px-6 py-3 bg-white border border-gray-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all">
                        Lihat Sitemap
                    </a>
                </div>
                ...
                <div class="p-8 bg-slate-50 rounded-[2rem] border border-gray-100 space-y-4">
                    <div class="h-12 w-12 bg-amber-500 rounded-xl flex items-center justify-center text-slate-900 shadow-lg shadow-amber-500/20">
                    ...
                    <a href="{{ route('admin.settings.backup') }}" class="inline-block px-6 py-3 bg-amber-500 text-slate-900 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-amber-400 transition-all shadow-lg shadow-amber-500/10">
                        Download Backup
                    </a>
                </div>
            </div>
        ...
        <!-- Tab: Identitas -->
        <div x-show="tab === 'identity'" x-transition class="bg-white p-8 md:p-12 rounded-xl border border-gray-100 shadow-sm space-y-8">
            <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <input type="text" name="site_name" value="{{ $settings['site_name'] ?? 'BisnisGrowth' }}" class="w-full bg-gray-50 border-none rounded-xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-amber-500">
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Tagline</label>
                    <input type="text" name="site_tagline" value="{{ $settings['site_tagline'] ?? 'Satu Link untuk Semua Bisnismu' }}" class="w-full bg-gray-50 border-none rounded-xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-amber-500">
                </div>
            </div>

            <div class="space-y-2">
                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Deskripsi Bisnis</label>
                <textarea name="site_description" rows="4" class="w-full bg-gray-50 border-none rounded-xl px-5 py-4 text-sm font-medium focus:ring-2 focus:ring-amber-500">{{ $settings['site_description'] ?? '' }}</textarea>
            </div>
        </div>

        <!-- Tab: SEO -->
        <div x-show="tab === 'seo'" x-transition class="bg-white p-8 md:p-12 rounded-xl border border-gray-100 shadow-sm space-y-8">
            <div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="ga_id" value="{{ $settings['ga_id'] ?? '' }}" placeholder="G-XXXXXXXXXX" class="w-full bg-gray-50 border-none rounded-xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-amber-500">
            </div>

            <div class="space-y-2">
                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Meta Keywords</label>
                <input type="text" name="meta_keywords" value="{{ $settings['meta_keywords'] ?? '' }}" placeholder="umkm, bisnis, edukasi, direktori" class="w-full bg-gray-50 border-none rounded-xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-amber-500">
            </div>
        </div>

        <!-- Tab: AI & API -->
        <div x-show="tab === 'api'" x-transition class="bg-white p-8 md:p-12 rounded-xl border border-gray-100 shadow-sm space-y-8">
            <div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="official_whatsapp" value="{{ $settings['official_whatsapp'] ?? '089515915699' }}" class="w-full bg-gray-50 border-none rounded-xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-amber-500">
        ...
        <div class="flex justify-end gap-4 pt-4">
            <button type="submit" class="bg-slate-900 text-white px-12 py-5 rounded-xl text-[10px] font-black uppercase tracking-[0.3em] hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/20">
