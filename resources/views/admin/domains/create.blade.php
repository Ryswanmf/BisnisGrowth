...
<div class="max-w-2xl mx-auto">
    ...
    <form action="{{ route('admin.domains.store') }}" method="POST" class="bg-white p-8 rounded-lg shadow-sm border border-gray-100 space-y-6">
        @csrf
        <div>
            <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Nama Domain</label>
            <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Bisnis Growth" class="w-full bg-gray-50 border-gray-100 rounded-lg px-4 py-3 focus:ring-amber-500 focus:border-amber-500 @error('name') border-red-500 @enderror" required>
        ...
        <div>
            ...
            <input type="url" name="url" value="{{ old('url') }}" placeholder="https://bisnisgrowth.com" class="w-full bg-gray-50 border-gray-100 rounded-lg px-4 py-3 focus:ring-amber-500 focus:border-amber-500 @error('url') border-red-500 @enderror" required>
