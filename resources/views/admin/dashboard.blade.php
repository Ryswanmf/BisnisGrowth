...
<div class="space-y-10">
    ...
    <div class="bg-slate-900 p-8 md:p-12 rounded-lg shadow-2xl relative overflow-hidden text-white group">
        ...
        <div class="relative z-10 max-w-2xl">
            ...
            <div class="flex flex-wrap gap-4">
                <a href="{{ route('admin.articles.create') }}" class="px-8 py-4 bg-amber-500 text-slate-900 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] hover:bg-amber-400 transition-all shadow-lg shadow-amber-500/20">Tulis Artikel Baru</a>
                <a href="{{ route('admin.traffic.index') }}" class="px-8 py-4 bg-white/5 border border-white/10 text-white rounded-lg text-[10px] font-black uppercase tracking-[0.2em] hover:bg-white/10 transition-all">Lihat Traffic</a>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
        ...
        <div class="bg-slate-900 p-8 rounded-lg border border-slate-800 shadow-sm hover:shadow-xl transition-all text-white lg:col-span-1 xl:col-span-2">
            ...
            <div class="space-y-4">
                @foreach($stats['top_domains']->take(2) as $index => $domain)
                    <div class="flex items-center justify-between gap-4 group">
                        <div class="min-w-0 flex-1">
                            <h5 class="text-xs font-bold truncate group-hover:text-amber-500 transition-colors">{{ $domain->name }}</h5>
                            <p class="text-[8px] text-slate-500 truncate">{{ str_replace(['http://', 'https://', 'www.'], '', $domain->url) }}</p>
                        </div>
                        <div class="text-right">
                            <span class="text-sm font-black text-amber-500">{{ number_format($domain->hits_count ?? 0) }}</span>
