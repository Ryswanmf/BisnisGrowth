...
<div class="space-y-10">
    ...
    <!-- Welcome Header -->
    <div class="bg-slate-900 p-8 md:p-12 rounded-[3rem] shadow-2xl relative overflow-hidden text-white group">
        ...
        <div class="relative z-10 max-w-2xl">
            ...
            <div class="flex flex-wrap gap-4">
                <a href="{{ route('admin.articles.create') }}" class="px-8 py-4 bg-amber-500 text-slate-900 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-amber-400 transition-all shadow-lg shadow-amber-500/20">Tulis Artikel Baru</a>
                <a href="{{ route('admin.traffic.index') }}" class="px-8 py-4 bg-white/5 border border-white/10 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-white/10 transition-all">Lihat Traffic</a>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div class="bg-white p-8 md:p-10 rounded-[3rem] border border-gray-100 shadow-sm">
            ...
            <div class="space-y-6">
                @foreach($stats['recent_articles'] as $art)
                    <div class="flex items-center gap-5 group">
                        <div class="h-14 w-14 bg-slate-50 rounded-2xl overflow-hidden shrink-0 border border-gray-100">
                            @if($art->image)
        ...
        <div class="bg-white p-8 md:p-10 rounded-[3rem] border border-gray-100 shadow-sm">
            ...
            <div class="space-y-6">
                @foreach($stats['top_performing_articles'] as $top)
                    <div class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl hover:bg-amber-50 transition-all border border-transparent hover:border-amber-100 group">
