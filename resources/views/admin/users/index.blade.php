        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        @if($user->role === 'admin')
                            <span class="text-amber-600 text-[10px] font-black uppercase tracking-widest bg-amber-50 px-3 py-1 rounded-lg border border-amber-100">Admin</span>
                        @else
                            <span class="text-slate-500 text-[10px] font-black uppercase tracking-widest bg-slate-50 px-3 py-1 rounded-lg border border-slate-100">User</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase">{{ $user->created_at->format('d M Y') }}</td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('admin.users.edit', $user) }}" class="p-2 bg-white border border-gray-100 rounded-lg text-gray-400 hover:text-amber-500 transition-all">
                            ...
                            <form id="delete-form-{{ $user->id }}" action="{{ route('admin.users.destroy', $user) }}" method="POST">
                                @csrf @method('DELETE')
                                <button type="button" onclick="confirmDelete('delete-form-{{ $user->id }}')" class="p-2 bg-white border border-gray-100 rounded-lg text-gray-400 hover:text-red-500 transition-all">
