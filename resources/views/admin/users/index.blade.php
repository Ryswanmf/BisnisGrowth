...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <a href="{{ route('admin.users.create') }}" class="bg-slate-900 text-white px-6 py-3 rounded-lg text-sm font-black flex items-center gap-2 hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/20">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('admin.users.edit', $user) }}" class="p-2 bg-white border border-gray-100 rounded-md text-gray-400 hover:text-amber-500 transition-all">
                            ...
                            <form id="delete-form-{{ $user->id }}" action="{{ route('admin.users.destroy', $user) }}" method="POST">
                                @csrf @method('DELETE')
                                <button type="button" onclick="confirmDelete('delete-form-{{ $user->id }}')" class="p-2 bg-white border border-gray-100 rounded-md text-gray-400 hover:text-red-500 transition-all">
