    ...
    <form action="{{ isset($user) ? route('admin.users.update', $user->id) : route('admin.users.store') }}" method="POST" class="space-y-8">
        ...
        <div class="bg-white p-8 rounded-xl border border-gray-100 shadow-sm space-y-6">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $user->name ?? '') }}" class="w-full bg-gray-50 border-none rounded-xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-amber-500" required>
            </div>

            <div>
                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Alamat Email</label>
                <input type="email" name="email" value="{{ old('email', $user->email ?? '') }}" class="w-full bg-gray-50 border-none rounded-xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-amber-500" required>
            </div>
            ...
            <div>
                ...
                <input type="password" name="password" class="w-full bg-gray-50 border-none rounded-xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-amber-500" {{ isset($user) ? '' : 'required' }}>
            </div>

            <div>
                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Role / Hak Akses</label>
                <select name="role" class="w-full bg-gray-50 border-none rounded-xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-amber-500 appearance-none">
        ...
        <div class="flex justify-end gap-4">
            ...
            <button type="submit" class="bg-slate-900 text-white px-10 py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-slate-800 transition-all shadow-lg">
