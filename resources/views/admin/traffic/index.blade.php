...
<div class="space-y-10">
    ...
    <div class="bg-white p-8 md:p-10 rounded-lg shadow-sm border border-gray-100">
        <div class="flex justify-between items-center mb-10">
            ...
            <a href="{{ route('admin.domains.index') }}" class="text-[10px] font-black text-amber-600 uppercase tracking-widest hover:underline">Kelola Domain</a>
        </div>
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach($topDomains as $index => $domain)
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            <td class="px-4 py-4">
                                <span class="flex items-center justify-center h-6 w-6 rounded-full {{ $index < 3 ? 'bg-amber-500 text-slate-900' : 'bg-slate-100 text-slate-400' }} text-[10px] font-black">
                                    {{ $index + 1 }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-4">
                                <code class="text-[10px] bg-slate-100 px-2 py-1 rounded text-slate-500 font-bold group-hover:bg-white transition-colors">{{ $domain->url }}</code>
                            </td>
