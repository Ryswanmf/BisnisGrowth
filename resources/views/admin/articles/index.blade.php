...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <a href="{{ route('admin.articles.create') }}" class="bg-amber-500 text-slate-900 px-6 py-3 rounded-xl text-sm font-black flex items-center gap-2 hover:bg-amber-400 transition-all shadow-lg shadow-amber-500/20">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                @if($article->image)
                                    <img src="{{ asset($article->image) }}" class="h-12 w-16 object-cover rounded-xl border border-gray-100">
                                @else
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                ...
                                <form id="delete-form-{{ $article->id }}" action="{{ route('admin.articles.destroy', $article) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="button" onclick="confirmDelete('delete-form-{{ $article->id }}')" class="p-2 bg-white border border-gray-100 rounded-lg text-gray-400 hover:text-red-500 transition-all">
