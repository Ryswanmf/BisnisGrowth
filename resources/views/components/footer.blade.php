    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-8">
                    ...
                    @if($footerSetting)
                        @foreach(['facebook_url' => 'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z', 
                            ...
                            @if($footerSetting->$field)
                                <a href="{{ $footerSetting->$field }}" target="_blank" class="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-gold-400 hover:text-burgundy-900 hover:-translate-y-1 hover:shadow-lg transition-all duration-300">
        ...
        <div class="mt-20 pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[10px] font-black uppercase tracking-[0.3em] text-burgundy-200/50">
            <div class="flex flex-col md:flex-row items-center gap-2 md:gap-4">
                <span>{{ $footerSetting->copyright_text ?? '© ' . date('Y') . ' BisnisGrowth. All rights reserved.' }}</span>
