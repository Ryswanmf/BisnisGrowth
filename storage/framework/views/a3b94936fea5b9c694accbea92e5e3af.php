...
<?php $__env->startSection('header', 'Manajemen Artikel'); ?>
...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <a href="<?php echo e(route('admin.articles.create')); ?>" class="bg-amber-500 text-slate-900 px-6 py-3 rounded-lg text-sm font-black flex items-center gap-2 hover:bg-amber-400 transition-all shadow-lg shadow-amber-500/20">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                <?php if($article->image): ?>
                                    <img src="<?php echo e(asset($article->image)); ?>" class="h-12 w-16 object-cover rounded-lg border border-gray-100">
                                <?php else: ?>
                                    <div class="h-12 w-16 bg-gray-50 rounded-lg flex items-center justify-center text-gray-200 font-black text-xs">BG</div>
                                <?php endif; ?>
                                <div class="min-w-0">
                                    <p class="text-sm font-bold text-slate-900 truncate group-hover:text-amber-600 transition-colors"><?php echo e($article->title); ?></p>
                                    <p class="text-[10px] text-gray-400 font-medium truncate italic">/artikel/<?php echo e($article->slug); ?></p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 bg-slate-100 text-slate-600 text-[9px] font-black uppercase tracking-widest rounded-md">
                                <?php echo e($article->category_name ?: 'Wawasan'); ?>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex gap-2">
                                ...
                                <div class="flex flex-col items-center min-w-[45px] px-2 py-1 bg-blue-50 rounded-lg border border-blue-100">
                                    ...
                                    <span class="text-[11px] font-black text-blue-600"><?php echo e(number_format($article->click_count)); ?></span>
                                </div>
                                ...
                                <div class="flex flex-col items-center min-w-[45px] px-2 py-1 bg-emerald-50 rounded-lg border border-emerald-100">
                                    ...
                                    <span class="text-[11px] font-black text-emerald-600"><?php echo e(number_format($article->whatsapp_clicks)); ?></span>
                                </div>
                                <!-- Phone Clicks (Gray) -->
                                <div class="flex flex-col items-center min-w-[45px] px-2 py-1 bg-slate-100 rounded-lg border border-slate-200">
                                    ...
                                    <span class="text-[11px] font-black text-slate-600"><?php echo e(number_format($article->phone_clicks)); ?></span>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <?php if($article->status === 'publish'): ?>
                                <?php if($article->published_at && $article->published_at->isFuture()): ?>
                                    ...
                                    <p class="text-[8px] text-amber-500 font-bold mt-1"><?php echo e($article->published_at->format('d M, H:i')); ?></p>
                                <?php else: ?>
                                ...
                                <?php endif; ?>
                            <?php elseif($article->status === 'private'): ?>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <a href="<?php echo e(route('admin.articles.edit', $article)); ?>" class="p-2 bg-white border border-gray-100 rounded-md text-gray-400 hover:text-amber-500 transition-all">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <form id="delete-form-<?php echo e($article->id); ?>" action="<?php echo e(route('admin.articles.destroy', $article)); ?>" method="POST">
                                    <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                    <button type="button" onclick="confirmDelete('delete-form-<?php echo e($article->id); ?>')" class="p-2 bg-white border border-gray-100 rounded-md text-gray-400 hover:text-red-500 transition-all">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\BisnisGrowth\resources\views/admin/articles/index.blade.php ENDPATH**/ ?>